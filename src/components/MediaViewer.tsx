...
const MediaViewer = ({
  ...
  useEffect(() => {
    const fetchThumbnail = async () => {
      if (data && data.universeId) {
        try {
          const res = await fetch(`/api/thumbnail?id=${data.universeId}`);
          const json = await res.json();

          if (json?.imageUrl) {
            setThumbnail(json.imageUrl);
          } else {
            setThumbnail(data.image ?? ""); // fallback
          }
        } catch (err) {
          setThumbnail(data.image ?? ""); // fallback on error
        }
      } else {
  ...
  return (
    <div
      className={`fixed w-screen h-screen top-0 z-50 before:absolute before:-z-10 before:opacity-90 before:h-full before:w-full before:bg-primary backdrop-blur-lg ${
        viewerVisible ? "block" : "hidden"
      }`}
    >
      <button
        onClick={handleClose}
        className="h-16 w-16 absolute right-0 top-0 text-4xl mr-6 font-base text-secondary"
      >
        X
      </button>
          ...
          {data ? (
            <>
              {data.video ? (
                ...
                <img
                  src={thumbnail}
                  className={`rounded-md shadow-image-view ${
                    data.isSmall
                      ? "h-3/5 w-[80%] sm:w-auto"
                      : `object-cover object-top h-auto w-[90%]" ${
                          data.isBig
                            ? "max-w-[1280px] sm:w-[90%] md:w-[80%]"
                            : "max-w-[700px] sm:w-[80%] md:w-[60%] max-h-[300px] sm:max-h-[400px] sm:h-auto"
                        }`
                  }`}
